...
struct LongNumber ReadBinFile(const char* file)
{
	...
	if (remainder != 0) //если файл не кратен 32битам, тогда в старший разряд заносим биты путем логического сдвига
	{
		unsigned char ch;
		number.size++;
...
void WriteTextFile(const char* file, struct LongNumber number)
{
	...
	while (number.size != 1 || number.pointer[0] != 0)
	{
		...
		for (i = number.size - 1; i >= 0; i--)
		{
			...
			carry = tmp - (long long int) number.pointer[i] * 1000000000;  //максимальное количество разрядов для переменной
		}
